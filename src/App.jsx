...
const migrateStrain = (s) => {
  if (s.tiers) return { ...s, inStock: s.inStock !== false }; // Ensures inStock is defined
  const t = {
  ...
  return { ...rest, tiers: t, inStock: true };
};
...
const countFlags = (strains, menuType) =>
  strains
    .filter(s => s.inStock !== false && TIER_ORDER.some(t => s.tiers?.[t]?.active && (menuType === 'eighths' ? s.tiers[t].eighths : s.tiers[t].halves)))
...
export default function MenuApp() {
  ...
  const openNewFlower = () => { 
    ...
    setForm({ 
      id: mkId(), type: 'H', name: '', thc: '', lineage: '', terpenes: '', flags: {}, blanks: {}, inStock: true,
      tiers: {
